...
Flags:
  --bind            Address to bind the server (default: 0.0.0.0)
  --api-port        Metadata API port (default: 8000)
  --master-host     Master hostname/IP to register with
...
Config file format (JSON):
  {
    "path_prefix": "/mnt/files/"
  }

Usage:
  python -m slave.main --master-host 192.168.1.5 --config /etc/packa/slave.json
"""
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Address to bind the server (default: 0.0.0.0)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--master-host",
        help="Master hostname/IP to register with (omit to run standalone)",
    )
    parser.add_argument(
